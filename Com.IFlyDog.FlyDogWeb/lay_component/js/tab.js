...
layui.config({
    ...
    Tab.prototype.tabDelete = function (data) {
        var id = $(parent.document).find("[lay-id=" + data.id + "]").find(".layui-tab-close").data("id");
        element.tabDelete(ELEM.tabFilter, id);
    }
    Tab.prototype.on = function (events, callback) {

    }
    Tab.prototype.title = function() {
        var that = this;
        
        return ELEM.titleBox.find(".layui-this").children("cite").text();
    }

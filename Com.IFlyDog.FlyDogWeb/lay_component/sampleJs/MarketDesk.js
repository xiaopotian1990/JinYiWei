$(function () {
    ...
    $(".data-addcustomer-pop").on("click", ".add-cu-submit", function () {
        ...
        paraObj.data = {
            ...
            StoreID: $(".smartaddStoreCategory").val()
            
        };
        var url = "/Customer/AddAsyncByMarketDesk";
...
//验证提交表单 !(/^1[3|4|5|8][0-9]\d{4,8}$/.test(sMobile))
var verify = function () {
    var cuname = $("[name='addcuName']").val();
    var addcuMobile = $("[name='addcuMobile']").val();
    var addcuStoreId = $(".smartaddStoreCategory").val();
    ...
    var addcustomerTool = $(".smartaddcustomerChannel").val();
    ...
    if ( !(/^1[3|4|5|8|7][0-9]\d{4,8}$/.test(addcuMobile))) {
        layer.msg("顾客联系方式不正确！", { icon: 5 });
        return false;
    }
    ...
    if (addcustomerTool === "-1") {
        layer.msg("请选择沟通方式！", { icon: 5 });
        return false;
    }
    return true;
}
...
var gettodayCustomerInfo = function () {
    var url = "/Customer/CustomerCreateTodayAsyncByMarketDesk";
    var paraObj = {};
    var result = ajaxProcess(url, paraObj).Data;

...
$("#taghtml")
    .ready(function () {
        var getinfoFunc = function () {
            var url = "/SmartTag/TagGet";
            var paratObj = new Object();
            var data = ajaxProcess(url, paratObj);

            var interText = doT.template($("#tag_template").text());
...
$("#tagAddbut")
    .on("click",
        function () {
            //输出模板引擎
            var innerText = doT.template($("#showtagAddInfo_template").text());
            var contentData = $("#showtagAddInfo_div").html(innerText());
            //页面层
            layer.open({
                type: 1,
                title: "添加信息",
                ...
                success: function (layero, index) {
                    ...
                    $(".dept_commit")
                        .on("click",
                            function () {
                                //获取页面值
                                var addContent = $("#tagtinfoAddContent").val();

                                if (addContent.length > 50) {
                                    layer.msg("标签长度不可大过50!抱歉!", { icon: 5, style: "color:red" });
                                    return false;
                                }
                                //获取到页面值,组装DTO赋值
                                var realData = {};
                                realData.Content = addContent;
...
$(document)
    .on("click",
        ...
        function () {
            ...
            //回填数据接口
            var url = "/SmartTag/TagEditGetByID";
            ...
            if (parseInt(ResultType) === 0) { //接口返回的消息
                ...
                //从模版输出
                var innerText = doT.template($("#showtagEditInfo_template").text());
                var contentData = $("#showtagEditInfo_div").html(innerText(data.Data));
                    ...
                    success: function (layero, index1) {
                        ...
                        $(".layui-btn.layui-btn-normal.dept_commit")
                            .on("click",
                                function () {
                                    //获取值
                                    var id = $("#tagtinfoEditID").val();
                                    var infoeditContent = $("#tagtinfoEditContent").val();


                                    if (infoeditContent.length > 50) {
                                    ...
                                    var dto = new Object();
                                    dto.ID = id;
                                    dto.Content = infoeditContent;

                                    //将Dto放进集合
                                    var paraObj = new Object();
                                    paraObj.data = dto;
                                    //接口地址
                                    var url = "/SmartTag/TagEditSubmit";
                                    //传参
                                    var data = ajaxProcess(url, paraObj);
                                    if (data) {
                                        if (parseInt(data.ResultType) === 0) { //请求成功返回

                                            $("#showtagEditInfo_div").html("");
                                            var message = data.Message;
                                            //关闭窗口
                                            layer.close(index1);
                                            //提示信息

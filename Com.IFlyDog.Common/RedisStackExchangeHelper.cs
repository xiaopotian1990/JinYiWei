...
namespace Com.IFlyDog.Common
{
    ...
    /// Redis操作
    /// </summary>
    public class NewRedisStackExchangeHelper
    {
        private int DbNum = Convert.ToInt32(ConfigurationManager.AppSettings["DefaultRedisDB"]);
        private readonly ConnectionMultiplexer _conn;
        public string CustomKey;

        #region 构造函数

        public NewRedisStackExchangeHelper()
                : this(null)
        {
        }

        public NewRedisStackExchangeHelper(string readWriteHosts)
        {
            _conn =
                string.IsNullOrWhiteSpace(readWriteHosts) ?
                NewRedisConnectionHelp.Instance :
                NewRedisConnectionHelp.GetConnectionMultiplexer(readWriteHosts);
        }
        ...
        {
            var prefixKey = CustomKey ?? NewRedisConnectionHelp.SysCustomKey;
